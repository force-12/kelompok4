...
#            (Diperbarui untuk menghilangkan jam pulang)
...
# --- Fungsi Operasi Storage (Tidak Berubah) ---
def upload_photo(file_bytes, file_name):
    try:
        ...
        if "Duplicate" in str(e.message): return supabase.storage.from_(PHOTO_BUCKET).get_public_url(file_name)
        st.error(f"Gagal mengunggah foto: {e.message}"); return None

def delete_photo(file_name):
    try:
        supabase_admin.storage.from_(PHOTO_BUCKET).remove([file_name]); return True
    except APIError as e:
        st.error(f"Gagal menghapus foto: {e.message}"); return False
...
# --- Fungsi Database (Fungsi Jam Disederhanakan) ---
def get_jam_settings():
    """Hanya mengambil jam masuk."""
    try:
...
def insert_record(record_id, nim, nama, jurusan, timestamp, photo_url, status, late_status, latitude, longitude):
    try:
        record_data = {'id': record_id, 'nim': nim, 'nama': nama, 'jurusan': jurusan, 'timestamp': timestamp, 
                       'photo_path': photo_url, 'status': status, 'late_status': late_status,
                       'latitude': latitude, 'longitude': longitude}
        supabase.table('attendance').insert(record_data).execute()
        return True
    except APIError as e:
        st.error(f"Error menyimpan absensi: {e.message}"); return False

# ... Fungsi lain tidak ada perubahan signifikan ...
...
def validate_admin(username, password):
    ...
    except APIError as e: return False
...
def fetch_all_records():
    ...
    except APIError as e: return pd.DataFrame()
...
def delete_record(record_id):
    try:
        ...
        supabase.table('attendance').delete().eq('id', record_id).execute(); return True
    except APIError as e: return False
...
def add_mahasiswa(nim, nama, jurusan, password):
    try:
        supabase.table('mahasiswa').insert({'nim': nim, 'nama': nama, 'jurusan': jurusan, 'password': password}).execute(); return True
    except APIError as e:
        if 'duplicate key' in e.message: st.error("NIM sudah ada.")
        else: st.error(f"Error: {e.message}"); return False

def delete_mahasiswa(nim):
    try:
        supabase.table('mahasiswa').delete().eq('nim', nim).execute(); return True
    except APIError as e: return False
